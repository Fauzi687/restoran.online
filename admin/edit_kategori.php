...
if(isset($_POST['update'])){
    ...
    mysqli_query($conn,"
        UPDATE kategori
        SET nama_kategori='$nama', status='$status'
...
<title>Edit Kategori — Resto App</title>
...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar-label {
    ...
    color: var(--text-muted); padding: 0 8px; margin: 16px 0 6px;
  }
  .nav-link {
    display: flex; align-items: center; gap: 10px;
  ...
  .form-card-header h6 { font-size: 13px; font-weight: 600; color: var(--text-muted); margin: 0; text-transform: uppercase; letter-spacing: .6px; }
  ...
  /* Info box */
  .info-box {
    background: var(--accent-dim); border: 1px solid rgba(124,92,252,.2);
    border-radius: var(--radius-sm); padding: 12px 16px;
    font-size: 13px; color: var(--accent-hover);
    display: flex; align-items: flex-start; gap: 9px;
    margin-bottom: 24px;
  }
  .info-box i { font-size: 15px; margin-top: 1px; flex-shrink: 0; }
...
<div class="topbar">
  <nav class="breadcrumb-nav">
    ...
    <span class="sep">/</span>
    <span class="current">Edit Kategori</span>
  </nav>
...
<div class="main">
  <div class="page-content">
    ...
    <div class="info-box">
      ...
      <span>Mengubah nama kategori akan berdampak pada semua menu yang terhubung dengan kategori ini.</span>
    </div>
          ...
          <div class="row g-4">
            ...
            <div class="col-12">
              ...
              <div class="char-counter">
                <span id="charCount"><?= strlen($row['nama_kategori']) ?></span>/100 karakter
              </div>
            </div>
            ...
            <div class="col-12">
              ...
              <div class="status-preview <?= $row['status'] ?>" id="statusBadge">
                <i class="bi <?= $row['status']=='aktif' ? 'bi-check-circle-fill' : 'bi-slash-circle-fill' ?>" id="statusIcon"></i>
                <span id="statusText"><?= ucfirst($row['status']) ?></span>
              </div>
            </div>
